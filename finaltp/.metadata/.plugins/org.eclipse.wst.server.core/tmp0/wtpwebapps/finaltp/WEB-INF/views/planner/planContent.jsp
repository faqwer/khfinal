...
function addpreply (){
	...
	$.ajax({
		...
		dataType : 'html',
		type : 'POST',
		data : "planner_idx="+${pdto.planner_idx}+"&writer_idx="+${pdto.writer_idx}+"&user_idx="+${user_idx}+"&content="+replycontent,
...
<body>
	...
	<div id="plandiv1" style="background-image: url('${pdto.coverimg}');">
		...
		<c:if test="${user_idx eq pdto.writer_idx }">
			<p>
				<input type="button" value="수정" onclick="javascript:location.href='planUpdataForm.do?planner_idx=${pdto.planner_idx}'">
				<input type="button" value="삭제" onclick="javascript:location.href='planDelete.do?planner_idx=${pdto.planner_idx}'">
			</p>
		</c:if>
			<p>
				<img id="pfimg" src="${mdto.profile_img}" width="80" height="100">
				${mdto.id }
			</p>
	</div>
	<div id="plandiv4">
