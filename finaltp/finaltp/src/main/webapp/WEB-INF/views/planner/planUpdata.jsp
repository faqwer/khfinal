...
<script src="/finaltp/js/planUpdata.js"></script>
		...
		$(document).ready(
				function() {
					...
					$('#savebt').click(function(e){
						...
						var startdaytext=${year}+'-'+${month}+'-'+${day};
						...
						console.log(${pdto.planner_idx});
						var formData = new FormData();
						formData.append("planner_idx",${pdto.planner_idx});
...
<body>
	...
	<div id="plandiv1"
		style="background-image: url('${pdto.coverimg}');">
		<p>
			<span id="subject">${pdto.subject }</span> <input id="subject_update"
	...
	<div id="plandiv2">
		...
		<div>
			...
			<p>
				<select id="thema">
					<option ${pdto.thema eq '맛집투어'?'selected':'' }>맛집투어</option>
					<option ${pdto.thema eq '명소관광'?'selected':'' }>명소관광</option>
					<option ${pdto.thema eq '액티비티'?'selected':'' }>액티비티</option>
					<option ${pdto.thema eq '미술관/박물관 투어'?'selected':'' }>미술관/박물관 투어</option>
					<option ${pdto.thema eq '가족여행'?'selected':'' }>가족여행</option>
				</select>
			</p>
		</div>
	...
	<div id="plandiv4">
		<p>
			<input id="plan_story_subject" type="text" value="${pdto.plan_story_subject}">
		</p>
		<p>
			<textarea id="plan_story_content" rows="" cols="">${pdto.plan_story_content}</textarea>
		</p>
