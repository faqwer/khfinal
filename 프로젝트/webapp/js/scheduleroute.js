...
function showroutesearch(){
	...
	$('#routesearch').css({'margin-left':Width,'margin-top':Height}); 
	$('#routesearch').show();
...
function listdelete(object){
	var parent=$(object).parent().parent()[0];
	parent.removeChild($(object).parent()[0]);
	removeMarker();
	var parentHeadId=$(parent).attr('id');
	var targetidarr=parentHeadId.split("-");
	var target=document.getElementById(targetidarr[0]+"-"+targetidarr[1]);
	scheduleclick(target);
...
function routesearch(object){
	...
	if(!$('#routesearch1').has('#floating-panel').length){
		...
		document.getElementById('routesearch1').prepend(control);
	}
	initMap();
	wrapWindowByMask();
	var Height = $(window).scrollTop()+200;  
	var Width = $(window).width()/2-250;  
	$('#routesearch1').css({'margin-left':Width,'margin-top':Height}); 
	$('#routesearch1').show();
}
...
function route(scheduleroute){
	...
	if(scheduleroute=="ALL"){
		for(var j=0;j<thd_th.length-1;j++){
			for(var i=0;i< tbd_tr.length; i++){
				...
				if($(schedule).has('#listdiv').length){
					...
					addschedulemarker(place_location,color[colorck],markernum);
					markernum++;
				}
			}
			colorck++;
		...
		colorck=Math.floor(Math.random() * 5);
		for(var i=0;i< tbd_tr.length; i++){
...
function addschedulemarker(place_location,colorcol,markernum) {
	schedulemarker = new google.maps.Marker({
		position : place_location,
		icon : {
			url : '/finaltp/img/marker/'+colorcol+'/'+markernum+'.png',
