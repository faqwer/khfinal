	...
	for(var w=0;w<listdivs.length;w++){
		for(var s=0;s<jsonschedule.plan.length;s++){
			...
			if(jsonschedule.plan[s].placeId==place_id&&overlap==0){
				var datadiv=$(listdivs[w]).children('#moneydiv')[0];
				var traffic=$(datadiv).children('#moneytraffic').val(jsonschedule.plan[s].traffic);
				var lodge=$(datadiv).children('#moneylodge').val(jsonschedule.plan[s].lodge);
				var admission=$(datadiv).children('#moneyadmission').val(jsonschedule.plan[s].admission);
				var food=$(datadiv).children('#moneyfood').val(jsonschedule.plan[s].food);
				var shopping=$(datadiv).children('#moneyshopping').val(jsonschedule.plan[s].shopping);
				var besides=$(datadiv).children('#moneybesides').val(jsonschedule.plan[s].besides);
...
function createElement(place){
	...
	listdiv.setAttribute("style","background-color: white;width:250px;height:50px;margin-top:-10px;overflow:visible;position: absolute;");
	listdiv.setAttribute("ondragstart","drag(this,event)");
